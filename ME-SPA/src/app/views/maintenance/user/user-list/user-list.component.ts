...
import { UserService } from '../../../../_core/_service/user.service';
...
export class UserListComponent implements OnInit {
  ...
  authorize(template: TemplateRef<any>, items) {
    debugger
    this.userID = items.user_ID;
    this.userName = items.user_Name;
    this.userService
      .GetRoleByUser(items.user_ID).subscribe(res => {
        this.modalUser = res;
      })
  ...
  authorizeSave() {
    this.userService.SaveRoleUser(this.modalUser).subscribe((res) => {
      if(res) {
        this.alertify.success("Save succsess");
        this.modalRef.hide();
      } else {
