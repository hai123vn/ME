...
import { SixsScoreRecordService } from '../../../../_core/_service/sixs-score-record.service';
import { FunctionUtility } from '../../../../_utility/function-utility';

@Component({
  selector: 'app-sixs-score-report',
  templateUrl: './sixs-score-report.component.html',
  styleUrls: ['./sixs-score-report.component.scss']
})
export class SixsScoreReportComponent implements OnInit {
  ...
  constructor(
    private scoreRecordService: SixsScoreRecordService,
  ...
  loadData() {
    ...
    this.scoreRecordService.search(this.pagination.currentPage, this.pagination.itemsPerPage, object).subscribe((res: PaginationResult<AuditRate6s[]>) => {
  ...
  exportExcelDetail(item) {
    this.scoreRecordService.exportExcelDetail(item.recordId);
  }
  clearSearch() {
    this.pdc = "all";
    this.building = "all";
