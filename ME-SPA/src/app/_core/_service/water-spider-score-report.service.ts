...
export class WaterSpiderScoreReportService {
  ...
  exportExcel(auditRateSearch?: AuditRateSearch) {
    return this.http.post(this.baseUrl + 'WaterSpiderRecord/ExportExcelWaterSpider', auditRateSearch, { responseType: 'blob' }).subscribe((result: Blob) => {
  ...
  exportExcelDetail(recordId: string) {
    return this.http.get(this.baseUrl + 'WaterSpiderRecord/ExportExcelScoreRecordDetail', {
      ...
      const link = document.createElement('a');
      const currentTime = new Date();
      const filename = 'WaterSpider_Score_Record_Detail_' + currentTime.getFullYear().toString() + (currentTime.getMonth() + 1) + currentTime.getDate() + currentTime.toLocaleTimeString().replace(/[ ]|[,]|[:]/g, '').trim() + '.xlsx';
      link.href = url;
      link.setAttribute('download', filename);
      document.body.appendChild(link);
      link.click();
  ...
  getAuditTypeByBrandWaterSpider(brand: string) {
