...
namespace ME_API.Controller
{
    ...
    public class AuditTypeController : ControllerBase
    {
        ...
        {
            var auditTypes = await _auditTypeService.GetAllAsyn();
        ...
        public async Task<IActionResult> GetAuditByAuditType(AuditType1FormDto formdata)
        {
        ...
        {
            var auditType = await _auditTypeService.GetWithPaginations(param);
            Response.AddPagination(auditType.CurrentPage, auditType.PageSize, auditType.TotalCount, auditType.TotalPages);
            return Ok(auditType);
        }
        ...
        public async Task<IActionResult> Update([FromBody] AuditTypeDto auditTypeDto)
        {
            if (await _auditTypeService.Update(auditTypeDto))
                return NoContent();
            return BadRequest($"Updating AuditType {auditTypeDto.Audit_Type_ID}");
        }
        ...
        public async Task<IActionResult> Upgrade(string audit_Type_ID)
        {
            if (await _auditTypeService.Upgrade(audit_Type_ID))
                return NoContent();
            return BadRequest($"Updating upgrade failed on save");
        }
    }
}